...
def distribute_features(dataframe, num_clients, alpha, num_informative_clients, num_informative_features, ignore_columns = []):
    good_feature_df = []
    while num_informative_features > 0:
        portion = math.floor(num_informative_features / num_informative_clients)
        num_informative_features -= portion
        num_informative_clients -=1
        to_pass_cols = list(df.iloc[:, :portion].columns)
        good_feature_df.append(df[to_pass_cols])
        dataframe.drop(columns=to_pass_cols, inplace=True)
    ...
    for i, item in enumerate(good_feature_df):
        dfs[i] = pd.concat([good_feature_df[i], dfs[i]], axis=1)
    return dfs
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('./split_nn_controlled_runs_config.csv')

    partition_alpha,batch_size,lr,wd,epochs,client_num_in_total,cut_layer,num_ln,client_num_in_total,number_of_features,number_of_informative_features,number_of_informative_clients,random_seed = list(df.iloc[args.config_id])
    ...
    args.number_of_features = int(number_of_features)
    args.number_of_informative_features = int(number_of_informative_features)
    args.number_of_informative_clients = int(number_of_informative_clients)
    ...
    wandb.init(
        project="fedml",
        name=args.run_name + '_Config_' + str(args.config_id) + '_DS_' + str(args.dataset_id) + '_Alice_06',
        config=args
    )
    ...
    total_num_features = args.number_of_features
    n_informative = args.number_of_informative_features
    n_redundant = total_num_features - n_informative
    X, y = make_classification(n_samples=5000, n_features=total_num_features, n_informative=n_informative, n_redundant=n_redundant, random_state=args.random_seed, n_classes=4, n_clusters_per_class=1, shuffle=False)
    df = pd.DataFrame(X)
    df['class'] = y
    target_label = 'class'
    ignore_attributes = []

    ignore_categorical =  ignore_attributes
    categorical_features = []
    numerical_features = list(range(0,total_num_features))
    ...
    distributed_dataframes = distribute_features(df, num_clients, alpha, args.number_of_informative_clients, args.number_of_informative_features, ignore_categorical)
