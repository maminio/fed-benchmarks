...
if __name__ == "__main__":
    ...
    df = pd.read_csv('./run-configs/SL_ALL_FIXED_PA_CL_8DS.csv')
    ...
    args.desc = 'All hyperparameters are fixed except partition alpha and cut-layer. 8 datasets (432 runs). Seed is fixed.'
    ...
    wandb.init(
        project="fedml",
        name=args.run_name + '_Config_' + str(args.config_id) + '_DS_' + str(args.dataset_id) + '_Alice_19',
        config=args
    )
